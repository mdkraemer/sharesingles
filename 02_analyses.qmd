...
Negative effect for extraversion that does not really depend on the set of control variables. The effect is a bit stronger when looking at people who were never in a committed relationship or never cohabited with a partner compared to those who were never married.
:::
...
lmer_cust_int_pers = function(outcome, predictor, moderator, df, random_slope=FALSE) {
  ...
  if (random_slope==TRUE) {
    form = as.formula(paste0(outcome, " ~ 1 + ", predictor, "*", moderator, 
                             " + (1 + ", predictor, " + ", moderator, " | country_str)"))
  }
  else {
...
outcomes <- as.list(c(rep("extraversion", 6), rep("agreeableness_w", 6), rep("conscientiousness_w", 6), 
                      rep("neuroticism", 6), rep("openness", 6), rep("life_satisfaction", 6), rep("quality_of_life", 6)))
...
slopes_fr <- rep(list(FALSE), 42) # for now without random slopes
...
```{r}
#| echo: true
summary_mods_expl_pers %>% filter(outcome=="extraversion") %>% kable(., digits = 3)
```

Summary of results: Agreeableness
```{r}
#| echo: true
summary_mods_expl_pers %>% filter(outcome=="agreeableness_w") %>% kable(., digits = 3)
```

Summary of results: Conscientiousness
```{r}
#| echo: true
summary_mods_expl_pers %>% filter(outcome=="conscientiousness_w") %>% kable(., digits = 3)
```

Summary of results: Neuroticism
```{r}
#| echo: true
summary_mods_expl_pers %>% filter(outcome=="neuroticism") %>% kable(., digits = 3)
```

Summary of results: Openness
```{r}
#| echo: true
summary_mods_expl_pers %>% filter(outcome=="openness") %>% kable(., digits = 3)
```
...
::: {.callout-note appearance="minimal"}
Only moderation that is consistently significant at *p* = .005 is for life satisfaction and age. 
:::
...
outcomes <- as.list(c(rep("extraversion", 9), rep("agreeableness_w", 9), rep("conscientiousness_w", 9), 
                      rep("neuroticism", 9), rep("openness", 9), rep("life_satisfaction", 9), rep("quality_of_life", 9)))
...
slopes_fr <- rep(list(FALSE), 63) # for now without random slopes
combinations <- list(outcomes=outcomes, predictors=predictors, moderators=moderators, df_names=df_names, slopes_fr=slopes_fr)
...
```{r}
#| echo: true
summary_mods_expl_cntr %>% filter(outcome=="agreeableness_w") %>% kable(., digits = 3)
```

Summary of results: Conscientiousness
```{r}
#| echo: true
summary_mods_expl_cntr %>% filter(outcome=="conscientiousness_w") %>% kable(., digits = 3)
```
...
::: {.callout-note appearance="minimal"}
Cross-level moderation effects that are significant at *p* = .005 are present for agreeableness and the proportion of singles, neuroticism and gender ratio as well as proportion of singles, life satisfaction and religiosity, and quality of life and gender ratio as well as proportion of singles (and also religiosity for those never partnered). 
:::